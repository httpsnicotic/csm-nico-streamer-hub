...
const SECTION_PANEL_IDS={live:"live",socials:"socials",culture:"cultureAdmin",featured:"featured",clips:"clips",upcoming:"upcoming",announcements:"announcement",sponsors:"sponsorsAdmin",about:"about",custom:"extras"};
...
function normalizeConfig(input){
  ...
  if(!c.sponsors||typeof c.sponsors!=="object")c.sponsors=clone(d.sponsors||{});
  if(!Array.isArray(c.sponsors.categories))c.sponsors.categories=clone(d.sponsors?.categories||[]);
  if(!Array.isArray(c.sponsors.deals))c.sponsors.deals=[];
  if(!Array.isArray(c.sponsors.vip))c.sponsors.vip=[];
  if(!Array.isArray(c.sponsors.partners))c.sponsors.partners=[]; c.heroLine1Size=Number(c.heroLine1Size||1); c.heroLine2Size=Number(c.heroLine2Size||1); ['homeAuto','vipAuto','partnersAuto','dealsAuto'].forEach(k=>{if(c.sponsors[k]==null)c.sponsors[k]=true}); c.sponsors.homeSpeed=Number(c.sponsors.homeSpeed||14); c.sponsors.vipSpeed=Number(c.sponsors.vipSpeed||4.8); c.sponsors.partnersSpeed=Number(c.sponsors.partnersSpeed||5.2); c.sponsors.dealsSpeed=Number(c.sponsors.dealsSpeed||5.5);
  ...
  c.sponsors.intro=clean(c.sponsors.intro);c.sponsors.categories.forEach((x,i)=>{if(x.enabled==null)x.enabled=true;if(!x.id)x.id=`cat_${i+1}`;if(!x.name)x.name=x.tag||`Sesión ${i+1}`;if(!x.accent)x.accent=c.theme?.primary||'#ff2db7';x.description=clean(x.description)});c.sponsors.deals.forEach(x=>{x.description=clean(x.description);if(x.enabled==null)x.enabled=true;if(x.discountEnabled==null)x.discountEnabled=!!x.discountCode;if(!x.category)x.category='general';if(!x.color&&x.glow)x.color=x.glow});c.sponsors.vip.forEach(x=>{if(x.enabled==null)x.enabled=true});c.sponsors.partners.forEach(x=>{if(x.enabled==null)x.enabled=true});
...
function sectionIndexById(id){return (draft.sectionItems||[]).findIndex(x=>x.id===id)}
function sectionItemById(id){return (draft.sectionItems||[]).find(x=>x.id===id)}
function sectionDataPath(item){const i=sectionIndexById(item?.id);return i>=0?`sectionItems.${i}.data`:''}
function isSectionClone(item){return !!item && item.id!==item.type}
function sectionEditorContext(type){
  const item=activeSectionItemId?sectionItemById(activeSectionItemId):null;
  if(item&&item.type===type&&isSectionClone(item)){
    if(!item.data)item.data=sectionSnapshot(type,draft);
    return {root:item.data,path:sectionDataPath(item),item,isClone:true};
  }
  return {root:draft,path:'',item:null,isClone:false};
}
function sectionHeader(ctx,type){ctx.root.sectionHeaders??={};ctx.root.sectionHeaders[type]??=clone(draft.sectionHeaders?.[type]||{});return ctx.root.sectionHeaders[type]}
function sectionSnapshot(type,c){
  const h=clone(c.sectionHeaders?.[type]||{});
  if(type==='live')return {sectionHeaders:{live:h},liveDescription:c.liveDescription,liveImage:c.liveImage,liveStats:clone(c.liveStats||[]),kickHandle:c.kickHandle,isLive:c.isLive};
  if(type==='socials')return {sectionHeaders:{socials:h},socialsDescription:c.socialsDescription,socialsMainCta:clone(c.socialsMainCta||{}),socials:clone(c.socials||[])};
  if(type==='culture')return {culture:clone(c.culture||{}),sectionHeaders:{culture:h}};
  if(type==='featured')return {sectionHeaders:{featured:h},featured:clone(c.featured||[])};
  if(type==='clips')return {sectionHeaders:{clips:h},clips:clone(c.clips||[])};
  if(type==='upcoming')return {sectionHeaders:{upcoming:h},nextStreamTitle:c.nextStreamTitle,nextStreamText:c.nextStreamText,nextStreamISO:c.nextStreamISO,nextStreamImage:c.nextStreamImage};
  if(type==='announcements')return {sectionHeaders:{announcements:h},announcement:clone(c.announcement||{}),announcements:clone(c.announcements||[])};
  if(type==='sponsors')return {sectionHeaders:{sponsors:h},sponsorCta:clone(c.sponsorCta||{}),sponsors:clone(c.sponsors||{})};
  if(type==='about')return {sectionHeaders:{about:h},about:clone(c.about||{}),finalCta:clone(c.finalCta||{})};
  if(type==='custom')return {customSections:clone(c.customSections||[])};
  return {};
}
function arrayImageField(basePath,i,field,value,label='Imagen'){
  return imageField(`${basePath}.${i}.${field}`,value,label);
}
function cloneNotice(item,type){
  const labels={live:'Kick / En vivo',socials:'Redes',culture:'Instagram / Cultura',featured:'Carrusel',clips:'Clips',upcoming:'Próximo stream',announcements:'Anuncio',sponsors:'Canjes / Patrocinadores',about:'Sobre mí',custom:'Extra'};
  return item?`<div class="clone-editor-note">Editando copia independiente: <b>${esc(item.name||labels[type]||type)}</b>. Lo que cambies aquí no modifica la sección original.</div>`:'';
}

function cloneablePathForType(type,path){
  const groups={
    live:['sectionHeaders.live.','isLive','kickHandle','liveDescription','liveImage','liveStats'],
    socials:['sectionHeaders.socials.','socialsDescription','socialsMainCta.','socials'],
    culture:['culture.'],
    featured:['sectionHeaders.featured.','featured'],
    clips:['sectionHeaders.clips.','clips'],
    upcoming:['sectionHeaders.upcoming.','nextStreamTitle','nextStreamText','nextStreamISO','nextStreamImage'],
    announcements:['sectionHeaders.announcements.','announcement.','announcements'],
    sponsors:['sectionHeaders.sponsors.','sponsorCta.','sponsors.'],
    about:['sectionHeaders.about.','about.','finalCta.'],
    custom:['customSections']
  };
  return (groups[type]||[]).some(prefix=>path===prefix||path.startsWith(prefix));
}
function effectivePathFor(path,panelId=activePanelId){
  const item=activeSectionItemId?sectionItemById(activeSectionItemId):null;
  if(!item||!isSectionClone(item)||SECTION_PANEL_IDS[item.type]!==panelId||!cloneablePathForType(item.type,path))return path;
  if(!item.data)item.data=sectionSnapshot(item.type,draft);
  return `${sectionDataPath(item)}.${path}`;
}
function refreshSimpleFields(scope=document){
  (scope||document).querySelectorAll('[data-path]').forEach(el=>{
    const panelId=el.closest('.panel')?.id||activePanelId;
    const realPath=effectivePathFor(el.dataset.path,panelId);
    el.dataset.effectivePath=realPath;
    const val=getByPath(draft,realPath);
...
function syncPathPeers(path,source){const val=getByPath(draft,path);document.querySelectorAll('[data-path]').forEach(el=>{if(el===source)return;if((el.dataset.effectivePath||el.dataset.path)!==path)return;if(el.type==='checkbox')el.checked=!!val;else el.value=val??''});document.querySelectorAll(`[data-theme-color="${CSS.escape(path)}"]`).forEach(el=>{if(/^#[0-9a-fA-F]{6}$/.test(String(val)))el.value=val});document.querySelectorAll(`[data-theme-text="${CSS.escape(path)}"]`).forEach(el=>el.value=val??'');updateRangeOutputs()}
function bindSimpleFields(){document.querySelectorAll('[data-path]').forEach(el=>{const panelId=el.closest('.panel')?.id||activePanelId;const path=el.dataset.path;const realPath=effectivePathFor(path,panelId);el.dataset.effectivePath=realPath;const val=getByPath(draft,realPath);if(document.activeElement!==el){if(el.type==='checkbox')el.checked=!!val;else el.value=val??'';}if(el.dataset.bound==='1')return;el.dataset.bound='1';const h=()=>{const p=el.dataset.effectivePath||el.dataset.path;const v=el.type==='checkbox'?el.checked:(el.dataset.number!==undefined||el.type==='number'||el.type==='range'?Number(el.value):el.value);setByPath(draft,p,v);syncPathPeers(p,el);previewSignature='';markDirty()};el.addEventListener(el.type==='checkbox'?'change':'input',h)});updateRangeOutputs()}
...
function renderSocials(){const ctx=sectionEditorContext('socials'),root=ctx.root,base=ctx.path?`${ctx.path}.socials`:'socials';root.socials??=[];const w=document.getElementById("socialEditor");w.innerHTML=cloneNotice(ctx.item,'socials');(root.socials||[]).forEach((s,i)=>{const d=document.createElement("div");d.className="subcard";d.innerHTML=`<div class="subcard-head"><h3>${esc(s.label||`Red ${i+1}`)}</h3><div class="sub-actions"><button class="icon-btn" data-up-social="${i}">↑</button><button class="icon-btn" data-down-social="${i}">↓</button><button class="icon-btn danger" data-remove-social="${i}">×</button></div></div><div class="subcard-grid"><label class="toggle-row full"><span>Activado</span><input type="checkbox" data-social="${i}" data-field="enabled" ${s.enabled!==false?"checked":""}></label><label>Etiqueta<input data-social="${i}" data-field="label" value="${esc(s.label)}"></label><label>Usuario / @<input data-social="${i}" data-field="handle" value="${esc(s.handle)}"></label><label class="full">URL<input data-social="${i}" data-field="url" value="${esc(s.url)}"></label><label>Color de hover<input data-social="${i}" data-field="color" value="${esc(s.color)}"></label><label>Icono corto<input data-social="${i}" data-field="icon" value="${esc(s.icon)}"></label>${imageField(`${base}.${i}.image`,s.image,'Imagen opcional de la red')}</div>`;w.appendChild(d)});w.querySelectorAll("[data-social]").forEach(e=>e.addEventListener(e.type==="checkbox"?"change":"input",()=>{root.socials[Number(e.dataset.social)][e.dataset.field]=e.type==="checkbox"?e.checked:e.value;markDirty()}));w.querySelectorAll("[data-remove-social]").forEach(b=>b.onclick=()=>{root.socials.splice(Number(b.dataset.removeSocial),1);renderSocials();bindImages();markDirty()});w.querySelectorAll('[data-up-social]').forEach(b=>b.onclick=()=>moveArray(root.socials,Number(b.dataset.upSocial),-1,()=>{renderSocials();bindImages()}));w.querySelectorAll('[data-down-social]').forEach(b=>b.onclick=()=>moveArray(root.socials,Number(b.dataset.downSocial),1,()=>{renderSocials();bindImages()}))}
...
function renderCultureEditor(){
  const ctx=sectionEditorContext('culture'), root=ctx.root, base=ctx.path?`${ctx.path}.culture.items`:'culture.items';root.culture??={items:[]};if(!Array.isArray(root.culture.items))root.culture.items=[];
  const w=document.getElementById('cultureEditor');if(!w)return;w.innerHTML=cloneNotice(ctx.item,'culture');
  (root.culture.items||[]).forEach((x,i)=>{
    const d=document.createElement('div');d.className='subcard';
    d.innerHTML=`<div class="subcard-head"><h3>Tarjeta cultura ${i+1}</h3><div class="sub-actions"><button class="icon-btn" data-up-culture="${i}">↑</button><button class="icon-btn" data-down-culture="${i}">↓</button><button class="icon-btn danger" data-remove-culture="${i}">×</button></div></div><div class="subcard-grid"><label class="toggle-row full"><span>Activado</span><input type="checkbox" data-culture="${i}" data-field="enabled" ${x.enabled!==false?'checked':''}></label><label>Red / plataforma<select data-culture="${i}" data-field="platform">${culturePlatformSelect(x.platform)}</select></label><label>Nombre visible de la red (opcional)<input data-culture="${i}" data-field="platformLabel" value="${esc(x.platformLabel||'')}" placeholder="Vacío = usa el nombre de la red"></label><label>Título<input data-culture="${i}" data-field="title" value="${esc(x.title||'')}"></label><label>Texto opcional<input data-culture="${i}" data-field="text" value="${esc(x.text||'')}"></label><label>Símbolo personalizado (solo si eliges Otra)<input data-culture="${i}" data-field="badge" value="${esc(x.badge||'')}" placeholder="Ej: ★"></label><label class="full">URL<input data-culture="${i}" data-field="url" value="${esc(x.url||'')}"></label>${imageField(`${base}.${i}.image`,x.image,'Imagen')}</div>`;
    w.appendChild(d)
  });
  w.querySelectorAll('[data-culture]').forEach(e=>e.addEventListener(e.type==='checkbox'?'change':(e.tagName==='SELECT'?'change':'input'),()=>{root.culture.items[Number(e.dataset.culture)][e.dataset.field]=e.type==='checkbox'?e.checked:e.value;markDirty();queueCulturePreview()}));
  w.querySelectorAll('[data-remove-culture]').forEach(b=>b.onclick=()=>{root.culture.items.splice(Number(b.dataset.removeCulture),1);renderCultureEditor();bindImages();markDirty();queueCulturePreview()});
  w.querySelectorAll('[data-up-culture]').forEach(b=>b.onclick=()=>moveArray(root.culture.items,Number(b.dataset.upCulture),-1,()=>{renderCultureEditor();bindImages();queueCulturePreview()}));
  w.querySelectorAll('[data-down-culture]').forEach(b=>b.onclick=()=>moveArray(root.culture.items,Number(b.dataset.downCulture),1,()=>{renderCultureEditor();bindImages();queueCulturePreview()}));
}
function renderFeatured(){
  const ctx=sectionEditorContext('featured'), root=ctx.root, base=ctx.path?`${ctx.path}.featured`:'featured';
...
function renderSponsors(){
  const ctx=sectionEditorContext('sponsors'), root=ctx.root;
  root.sponsors??={categories:[],deals:[],vip:[],partners:[]};
  root.sponsors.categories??=[];root.sponsors.deals??=[];root.sponsors.vip??=[];root.sponsors.partners??=[];
  const base=ctx.path?`${ctx.path}.sponsors`:'sponsors';
  const w=document.getElementById('sponsorsEditor');if(!w)return;
  const input=(path,label,value,full='')=>`<label class="${full}">${label}<input data-sponsor-path="${path}" value="${esc(value||'')}"></label>`;
  const color=(path,label,value)=>`<label>${label}<input type="color" data-sponsor-path="${path}" value="${esc(value||'#ff2db7')}"></label>`;
  const area=(path,label,value)=>`<label class="full">${label}<textarea data-sponsor-path="${path}">${esc(value||'')}</textarea></label>`;
  const check=(path,label,value)=>`<label class="toggle-row"><span>${label}</span><input type="checkbox" data-sponsor-path="${path}" ${value!==false?'checked':''}></label>`;
  const categoryOptions = (selected='') => `<option value="general" ${selected==='general'?'selected':''}>General / Sin sesión</option>` + (root.sponsors.categories||[]).map(c=>`<option value="${esc(c.id||c.name)}" ${(selected===(c.id||c.name))?'selected':''}>${esc(c.name||c.tag||c.id)}</option>`).join('');
  const selectCategory=(path,label,value)=>`<label>${label}<select data-sponsor-path="${path}">${categoryOptions(value||'general')}</select></label>`;
  let html=cloneNotice(ctx.item,'sponsors');
  const sponsorCtaBasePath = ctx.path?`${ctx.path}.sponsorCta`:'sponsorCta';
  setByPath(draft,`${sponsorCtaBasePath}.url`,'#sponsors');
  html+=`<div class="subcard"><div class="subcard-head"><h3>Botón + portada</h3></div><div class="subcard-grid">${check(`${sponsorCtaBasePath}.enabled`,'Mostrar botón flotante en inicio',root.sponsorCta?.enabled)}${input(`${sponsorCtaBasePath}.label`,'Texto botón flotante',root.sponsorCta?.label)}<div class="helper full">Abre Canjes + Patrocinadores como vista interna encima de la misma página.</div></div></div>`;
  html+=`<div class="subcard"><div class="subcard-head"><h3>Portada de canjes</h3></div><div class="subcard-grid">${check(`${base}.enabled`,'Mostrar sección',root.sponsors.enabled)}${input(ctx.path?`${ctx.path}.sectionHeaders.sponsors.tag`:'sectionHeaders.sponsors.tag','Tag del título',root.sectionHeaders?.sponsors?.tag)}${input(ctx.path?`${ctx.path}.sectionHeaders.sponsors.title1`:'sectionHeaders.sponsors.title1','Título blanco',root.sectionHeaders?.sponsors?.title1)}${input(ctx.path?`${ctx.path}.sectionHeaders.sponsors.title2`:'sectionHeaders.sponsors.title2','Título neon',root.sectionHeaders?.sponsors?.title2)}${area(`${base}.intro`,'Texto introductorio',root.sponsors.intro)}${input(`${base}.backLabel`,'Botón volver',root.sponsors.backLabel)}${input(`${base}.dealsTitle`,'Título canjes',root.sponsors.dealsTitle)}${input(`${base}.dealsKicker`,'Tag canjes',root.sponsors.dealsKicker)}${input(`${base}.vipTitle`,'Título VIP',root.sponsors.vipTitle)}${input(`${base}.vipKicker`,'Tag VIP',root.sponsors.vipKicker)}${color(`${base}.vipAccent`,'Color VIP',root.sponsors.vipAccent||root.sponsors.themeColor||'#ff2db7')}${input(`${base}.partnersTitle`,'Título patrocinadores',root.sponsors.partnersTitle)}${input(`${base}.partnersKicker`,'Tag patrocinadores',root.sponsors.partnersKicker)}${color(`${base}.partnerAccent`,'Color patrocinadores',root.sponsors.partnerAccent||root.sponsors.vipAccent||'#ff2db7')}${color(`${base}.logoBackplate`,'Fondo logos PNG',root.sponsors.logoBackplate||'#171018')}${check(`${base}.homeAuto`,'Inicio: automático (sin flechas)',root.sponsors.homeAuto)}${input(`${base}.homeSpeed`,'Inicio: rapidez segundos',root.sponsors.homeSpeed||14)}${check(`${base}.vipAuto`,'VIP: automático (si apagas usa flechas)',root.sponsors.vipAuto)}${input(`${base}.vipSpeed`,'VIP: rapidez segundos',root.sponsors.vipSpeed||4.8)}${check(`${base}.partnersAuto`,'Patrocinadores: automático (si apagas usa flechas)',root.sponsors.partnersAuto)}${input(`${base}.partnersSpeed`,'Patrocinadores: rapidez segundos',root.sponsors.partnersSpeed||5.2)}${check(`${base}.dealsAuto`,'Canjes: automático (si apagas usa flechas)',root.sponsors.dealsAuto)}${input(`${base}.dealsSpeed`,'Canjes: rapidez segundos',root.sponsors.dealsSpeed||5.5)}</div></div>`;

  html+=`<div class="sponsor-admin-head"><h3>Sesiones de canjes</h3><button class="mini-btn" id="addSponsorCategoryBtn" type="button">+ SESIÓN</button></div><div class="helper">Crea sesiones como Restaurantes, Ropa, Barbería, Tecnología, etc. Luego asigna cada canje a una sesión.</div>`;
  root.sponsors.categories.forEach((x,i)=>{html+=`<div class="subcard sponsor-admin-compact"><div class="subcard-head"><h3>Sesión ${i+1}</h3><div class="sub-actions"><button class="icon-btn" data-up-sponsor-category="${i}">↑</button><button class="icon-btn" data-down-sponsor-category="${i}">↓</button><button class="icon-btn danger" data-remove-sponsor-category="${i}">×</button></div></div><div class="subcard-grid">${check(`${base}.categories.${i}.enabled`,'Activada',x.enabled)}${input(`${base}.categories.${i}.name`,'Nombre visible',x.name)}${input(`${base}.categories.${i}.id`,'ID interno',x.id)}${input(`${base}.categories.${i}.tag`,'Etiqueta pequeña',x.tag)}${color(`${base}.categories.${i}.accent`,'Color de sesión',x.accent)}${area(`${base}.categories.${i}.description`,'Descripción corta',x.description)}</div></div>`});

  html+=`<div class="sponsor-admin-head"><h3>Canjes activos</h3><button class="mini-btn" id="addSponsorDealBtn" type="button">+ CANJE</button></div>`;
  root.sponsors.deals.forEach((x,i)=>{html+=`<div class="subcard"><div class="subcard-head"><h3>Canje ${i+1}</h3><div class="sub-actions"><button class="icon-btn" data-up-sponsor-deal="${i}">↑</button><button class="icon-btn" data-down-sponsor-deal="${i}">↓</button><button class="icon-btn danger" data-remove-sponsor-deal="${i}">×</button></div></div><div class="subcard-grid">${check(`${base}.deals.${i}.enabled`,'Activado',x.enabled)}${selectCategory(`${base}.deals.${i}.category`,'Sesión donde aparece',x.category)}${input(`${base}.deals.${i}.tag`,'Etiqueta',x.tag)}${input(`${base}.deals.${i}.tier`,'Etiqueta 2',x.tier)}${input(`${base}.deals.${i}.name`,'Nombre del negocio',x.name)}${input(`${base}.deals.${i}.title`,'Título de la tarjeta',x.title)}${color(`${base}.deals.${i}.color`,'Color de borde/neón',x.color||x.glow||'#ff2db7')}${area(`${base}.deals.${i}.description`,'Descripción',x.description)}${input(`${base}.deals.${i}.url`,'Link directo',x.url)}${check(`${base}.deals.${i}.discountEnabled`,'Mostrar código de descuento',x.discountEnabled)}${input(`${base}.deals.${i}.discountLabel`,'Texto del código',x.discountLabel)}${input(`${base}.deals.${i}.discountCode`,'Código descuento',x.discountCode)}${imageField(`${base}.deals.${i}.image`,x.image,'Foto del negocio / producto')}${imageField(`${base}.deals.${i}.logo`,x.logo,'Logo del negocio')}</div></div>`});
  html+=`<div class="sponsor-admin-head"><h3>Patrocinadores VIP</h3><button class="mini-btn" id="addSponsorVipBtn" type="button">+ VIP</button></div>`;
  root.sponsors.vip.forEach((x,i)=>{html+=`<div class="subcard"><div class="subcard-head"><h3>VIP ${i+1}</h3><div class="sub-actions"><button class="icon-btn" data-up-sponsor-vip="${i}">↑</button><button class="icon-btn" data-down-sponsor-vip="${i}">↓</button><button class="icon-btn danger" data-remove-sponsor-vip="${i}">×</button></div></div><div class="subcard-grid">${check(`${base}.vip.${i}.enabled`,'Activado',x.enabled)}${input(`${base}.vip.${i}.name`,'Nombre',x.name)}${input(`${base}.vip.${i}.tag`,'Etiqueta',x.tag)}${input(`${base}.vip.${i}.url`,'Link',x.url)}${imageField(`${base}.vip.${i}.logo`,x.logo,'Logo VIP')}</div></div>`});
  html+=`<div class="sponsor-admin-head"><h3>Logos patrocinadores</h3><button class="mini-btn" id="addSponsorPartnerBtn" type="button">+ LOGO</button></div>`;
  root.sponsors.partners.forEach((x,i)=>{html+=`<div class="subcard"><div class="subcard-head"><h3>Logo ${i+1}</h3><div class="sub-actions"><button class="icon-btn" data-up-sponsor-partner="${i}">↑</button><button class="icon-btn" data-down-sponsor-partner="${i}">↓</button><button class="icon-btn danger" data-remove-sponsor-partner="${i}">×</button></div></div><div class="subcard-grid">${check(`${base}.partners.${i}.enabled`,'Activado',x.enabled)}${input(`${base}.partners.${i}.name`,'Nombre',x.name)}${input(`${base}.partners.${i}.tag`,'Etiqueta',x.tag)}${input(`${base}.partners.${i}.url`,'Link',x.url)}${imageField(`${base}.partners.${i}.logo`,x.logo,'Logo patrocinador')}</div></div>`});
  w.innerHTML=html;
  w.querySelectorAll('[data-sponsor-path]').forEach(e=>e.addEventListener((e.type==='checkbox'||e.tagName==='SELECT'||e.type==='color')?'change':'input',()=>{setByPath(draft,e.dataset.sponsorPath,e.type==='checkbox'?e.checked:e.value);previewSignature='';markDirty()}));
  const add=(arr,obj)=>{arr.push(obj);renderSponsors();bindImages();markDirty()};
  w.querySelector('#addSponsorCategoryBtn')?.addEventListener('click',()=>add(root.sponsors.categories,{id:uid('cat'),enabled:true,name:'Nueva sesión',tag:'CANJES',accent:'#ff2db7',description:''}));
  w.querySelector('#addSponsorDealBtn')?.addEventListener('click',()=>add(root.sponsors.deals,{id:uid('deal'),enabled:true,category:(root.sponsors.categories[0]?.id||'general'),tag:'CANJE',tier:'LOCAL',name:'NUEVO NEGOCIO',title:'Nuevo canje',description:'',image:'',logo:'',url:'#',color:'#ff2db7',discountEnabled:false,discountLabel:'CÓDIGO',discountCode:''}));
  w.querySelector('#addSponsorVipBtn')?.addEventListener('click',()=>add(root.sponsors.vip,{id:uid('vip'),enabled:true,name:'NUEVO VIP',tag:'VIP',logo:'',url:'#'}));
  w.querySelector('#addSponsorPartnerBtn')?.addEventListener('click',()=>add(root.sponsors.partners,{id:uid('partner'),enabled:true,name:'NUEVO SPONSOR',tag:'OFICIAL',logo:'',url:'#'}));
  ['category','deal','vip','partner'].forEach(kind=>{
    const key=kind==='deal'?'deals':(kind==='partner'?'partners':(kind==='category'?'categories':kind));
    w.querySelectorAll(`[data-remove-sponsor-${kind}]`).forEach(b=>b.onclick=()=>{root.sponsors[key].splice(Number(b.dataset[`removeSponsor${kind[0].toUpperCase()+kind.slice(1)}`]),1);setByPath(draft,`${base}.${key}`,root.sponsors[key]);renderSponsors();bindImages();markDirty()});
    w.querySelectorAll(`[data-up-sponsor-${kind}]`).forEach(b=>b.onclick=()=>moveArray(root.sponsors[key],Number(b.dataset[`upSponsor${kind[0].toUpperCase()+kind.slice(1)}`]),-1,()=>{setByPath(draft,`${base}.${key}`,root.sponsors[key]);renderSponsors();bindImages()}));
    w.querySelectorAll(`[data-down-sponsor-${kind}]`).forEach(b=>b.onclick=()=>moveArray(root.sponsors[key],Number(b.dataset[`downSponsor${kind[0].toUpperCase()+kind.slice(1)}`]),1,()=>{setByPath(draft,`${base}.${key}`,root.sponsors[key]);renderSponsors();bindImages()}));
  });
}
...
function renderSections(){
  ...
  (draft.sectionItems||[]).forEach((item,i)=>{
    item.style??={};
    const isCopy=isSectionClone(item);
    const isCarousel=item.type==='featured';
  ...
  w.querySelectorAll("[data-section-style]").forEach(e=>e.addEventListener('input',()=>{const item=draft.sectionItems.find(x=>x.id===e.dataset.sectionStyle);if(!item||item.type!=='featured')return;item.style??={};item.style[e.dataset.styleField]=e.type==='range'?Number(e.value):e.value;previewSignature="";markDirty()}));
  w.querySelectorAll("[data-duplicate-section]").forEach(b=>b.onclick=()=>{const i=Number(b.dataset.duplicateSection),source=draft.sectionItems[i];if(!source)return;const copy={...clone(source),id:uid("section"),name:`${source.name} copia`,enabled:source.enabled!==false,style:clone(source.style||{}),data:clone(source.data||sectionSnapshot(source.type,draft))};draft.sectionItems.splice(i+1,0,copy);draft.sectionOrder=draft.sectionItems.map(x=>x.type);renderSections();renderAdminNavigation();previewSignature="";markDirty();status("Sección duplicada independiente ✓")});
  w.querySelectorAll("[data-delete-section]").forEach(b=>b.onclick=()=>{const i=Number(b.dataset.deleteSection),item=draft.sectionItems[i];if(!item||!isSectionClone(item)){status("La sección original no se elimina. Solo copias.",false);return}const wasActive=activeSectionItemId===item.id;draft.sectionItems.splice(i,1);draft.sectionOrder=draft.sectionItems.map(x=>x.type);if(wasActive){activeSectionItemId='';activeAdminNavKey='core-sections'}renderSections();renderAdminNavigation();previewSignature="";markDirty();status("Copia eliminada ✓")});
...
async function handleUpload(file,path){if(!file)return;try{status("Procesando imagen...");let url=window.CloudConfig.enabled()?await window.CloudConfig.uploadImage(file):await fileToDataUrl(file);if(file.type==="image/png" && !String(url).startsWith('data:image/png') && !String(url).includes('#png')) url=String(url)+'#png';setByPath(draft,path,url);renderAll();markDirty();if(String(path||"").startsWith("culture.items.")){activePanelId="cultureAdmin";queueCulturePreview()}status("Imagen lista. Pulsa VISTA PREVIA para revisarla ✓")}catch(e){status(e.message,false)}}
function bindImages(){document.querySelectorAll("[data-upload-target]").forEach(e=>{const panelId=e.closest('.panel')?.id||activePanelId;e.dataset.effectiveUploadPath=effectivePathFor(e.dataset.uploadTarget,panelId);if(e.dataset.imgBound)return;e.dataset.imgBound="1";e.onchange=()=>handleUpload(e.files?.[0],e.dataset.effectiveUploadPath||e.dataset.uploadTarget)});document.querySelectorAll("[data-array-image-path]").forEach(e=>{if(e.dataset.imgBound)return;e.dataset.imgBound="1";e.oninput=()=>{setByPath(draft,e.dataset.arrayImagePath,e.value);previewSignature="";markDirty();if(String(e.dataset.arrayImagePath||"").includes("culture.items.")){activePanelId="cultureAdmin";queueCulturePreview()}}});document.querySelectorAll("[data-array-upload-path]").forEach(e=>{if(e.dataset.imgBound)return;e.dataset.imgBound="1";e.onchange=()=>handleUpload(e.files?.[0],e.dataset.arrayUploadPath)})}
...
function renderAll(){document.querySelectorAll("[data-path]").forEach(e=>delete e.dataset.bound);bindSimpleFields();renderTheme();renderStats();renderNav();renderSocials();renderCultureEditor();renderSections();renderFeatured();renderClips();renderAnnouncements();renderSponsors();renderCustomSections();renderAdminNavigation();bindImages();updateActionState()}
...
const PANEL_TO_PREVIEW={general:'#hero',nav:'#hero',theme:'#hero',effects:'#hero',live:'#liveSection',socials:'#socials',cultureAdmin:'#culture',featured:'#featured',clips:'#clips',upcoming:'#upcoming',announcement:'#announcements',sponsorsAdmin:'#sponsors',about:'#about',extras:'#customSectionContainer',sections:'#dynamicSections',history:'footer'};
const PANEL_LABELS={general:'PORTADA',nav:'PORTADA',theme:'PORTADA',effects:'PORTADA',live:'KICK / EN VIVO',socials:'REDES',cultureAdmin:'INSTAGRAM',featured:'DESTACADO',clips:'CLIPS',upcoming:'PRÓXIMO STREAM',announcement:'AVISOS',sponsorsAdmin:'CANJES',about:'SOBRE MÍ',extras:'EXTRA',sections:'SECCIONES',history:'PIE / HISTORIAL'};
...
function installPreviewSync(){
  ...
  nav?.addEventListener('click',e=>{
    ...
    refreshSimpleFields(panel);renderStats();renderSocials();renderCultureEditor();renderFeatured();renderClips();renderAnnouncements();renderSponsors();renderCustomSections();renderSections();bindImages();
...
document.getElementById("discardBtn").onclick=async()=>{draft=clone(published);await window.LocalConfigDB.remove(PREVIEW_KEY);renderAll();markClean();status("Borrador descartado.")};document.getElementById("addNavBtn").onclick=()=>{draft.navLinks.push({id:uid('nav'),label:'NUEVO ENLACE',target:'#',enabled:true});renderNav();previewSignature='';markDirty()};document.getElementById('addCultureItemBtn').onclick=()=>{const ctx=sectionEditorContext('culture');ctx.root.culture??={items:[]};ctx.root.culture.items??=[];ctx.root.culture.items.push({id:uid('ig'),title:'NUEVA FOTO',text:'',platform:'instagram',platformLabel:'',badge:'',image:'',url:ctx.root.culture.ctaUrl||draft.culture.ctaUrl||'#',enabled:true});renderCultureEditor();bindImages();previewSignature='';markDirty();activePanelId='cultureAdmin';queueCulturePreview()};document.getElementById("addSocialBtn").onclick=()=>{const ctx=sectionEditorContext('socials');ctx.root.socials??=[];ctx.root.socials.push({id:uid("social"),label:"Nueva red",handle:"@handle",url:"#",color:"#ffffff",icon:"+",image:"",enabled:true});renderSocials();bindImages();previewSignature="";markDirty()};document.getElementById("addFeaturedBtn").onclick=()=>{const ctx=sectionEditorContext('featured');ctx.root.featured??=[];ctx.root.featured.push({id:uid("feature"),kicker:"NEW",title:"NUEVA TARJETA",subtitle:"",url:"#",image:"",badgeImage:"",glow:"#ff2db7",enabled:true});renderFeatured();bindImages();previewSignature="";markDirty()};document.getElementById("addClipBtn").onclick=()=>{const ctx=sectionEditorContext('clips');ctx.root.clips??=[];ctx.root.clips.push({id:uid("clip"),title:"NUEVO CLIP",subtitle:"",url:"#",image:"",enabled:true});renderClips();bindImages();previewSignature="";markDirty()};document.getElementById('addAnnouncementBtn').onclick=()=>{const ctx=sectionEditorContext('announcements');ctx.root.announcements??=[];ctx.root.announcements.push({id:uid('notice'),enabled:true,kicker:'AVISO',title:'NUEVO AVISO',chip:'',text:'',cta:'',url:'#',image:''});renderAnnouncements();bindImages();markDirty()};document.getElementById('addCustomSectionBtn').onclick=()=>{const ctx=sectionEditorContext('custom');ctx.root.customSections??=[];ctx.root.customSections.push({id:uid('custom'),kicker:'NUEVA SECCIÓN',title1:'NUEVO',title2:'BLOQUE.',text:'',ctaLabel:'ABRIR',ctaUrl:'#',image:'',enabled:true,anchorId:'',ctaEnabled:true});renderCustomSections();bindImages();previewSignature='';markDirty()};document.getElementById("refreshHistoryBtn").onclick=loadHistory;
